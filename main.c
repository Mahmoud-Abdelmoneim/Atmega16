...
keypad_t keypad={
	...
	.keypad_column_pin[1].port=PORTB_INDEX,
	...
	.keypad_column_pin[2].port=PORTB_INDEX,
	...
	.keypad_column_pin[3].port=PORTB_INDEX,
...
int main(void)
{
	...
	if(x==NOTPRESSED)
	{
		...
		for(i=0;i<=3;i++)
		{
			...
			_delay_ms(500);
			lcd_4bit_send_char_data_pos(&lcd1,1,13+i,'*');
			_delay_ms(500);
    ...
    while (flag==0) 
    {
		...
		lcd_4bit_send_string_pos(&lcd1,1,2," check password");
		...
		if(location1 == arr[0] && location2 == arr[1] 
			&& location3 == arr[2] && location4 == arr[3])
		{
			lcd_4bit_send_comand(&lcd1,_LCD_CLEAR);
			_delay_ms(100);
			lcd_4bit_send_string_pos(&lcd1,1,3,"correct password");	
			_delay_ms(100);
			lcd_4bit_send_string_pos(&lcd1,2,3,"safe will open now");
			flag=1;			
		}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					flag=1;
					lcd_4bit_send_comand(&lcd1,_LCD_CLEAR);
					lcd_4bit_send_string_pos(&lcd1,1,1," reset the safe");
					for(uint8 j=0;j<5;j++)
					{
